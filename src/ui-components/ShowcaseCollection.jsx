...
import { listChallengesInfos } from "../graphql/queries";
...
export default function ShowcaseCollection(props) {
    ...
    while ((newCache.length < cacheUntil || !isPaginated) && newNext != null) {
      ...
      const result = (
        await client.graphql({
          query: listChallengesInfos.replaceAll("__typename", ""),
          variables,
        })
      ).data.listChallengesInfos;
  ...
  return (
    <div>
      <Collection
        ...
        {...getOverrideProps(overrides, "ShowcaseCollection")}
        {...rest}
      >
        {(item, index) => {
          if (loading) {
